...
def KNN_algorithm(data_type[:,::1] x_train, label_type[::1] class_train, data_type[:,::1] x_test, Py_ssize_t k):
    ...
    if label_type is int:
        dtype = np.intc
    elif label_type is double:
        dtype = np.double
    elif label_type is cython.longlong:
        dtype = np.longlong

    class_test_pred = np.zeros(x_test_max, dtype = dtype)
    cdef label_type[::1] class_test_pred_view = class_test_pred
    ...
    cdef label_type[::1] nearest_neighbor_labels_view = nearest_neighbor_labels
